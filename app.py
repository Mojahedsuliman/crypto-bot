...
def evaluate_old_signals():
    """تقييم الإشارات القديمة بعد 24 ساعة"""
...
def get_precise_analysis(symbol):
    try:
        ...
        # تسجيل الإشارة إذا كانت قوية
        signal_id = None
        if signal != "NEUTRAL" and abs_score >= 2:
            signal_id = log_signal(symbol, signal, score, current_price)
...
✅ يتعلم من كل إشارة
✅ نسبة النجاح الحالية: {performance['accuracy']:.1%}
"""
...
⚡ سرعة التحليل: 3-5 ثوانٍ

💡 البوت يتعلم من كل إشارة ويزداد دقة!
"""
    bot.reply_to(message, msg, parse_mode='Markdown')
...
📊 نسبة النجاح الحالية: `{performance['accuracy']:.1%}`
📈 إجمالي الإشارات: `{performance['total']}`

🔍 *كيف يتعلم؟*
• يسجل كل إشارة يرسلها
• يقيمها بعد 24 ساعة
• يحسب نسبة النجاح الفعلية
• يعدل توقعاته بناءً على النتائج

🎯 الهدف: الوصول إلى 85%+ دقة
"""
    bot.reply_to(message, msg, parse_mode='Markdown')
...
def callback(call):
    ...
    if chat_id not in bot_data:
        bot.answer_callback_query(call.id, "انتهت الصلاحية")
        return
...
try:
    bot.send_message(CHAT_ID, f"🎯 *البوت المتعلم بدأ العمل!*\n📊 يحلل {len(MAIN_COINS)} عملة\n📈 نسبة النجاح: {performance['accuracy']:.1%}\n💡 أرسل `/daily`", parse_mode='Markdown')
except:
    pass
